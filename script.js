//  5d24449a50d14cf393d8108c149ed1b3
const guldur = document.querySelector('button')
...
async function zarafatiYarat() {
    let showJoke = document.getElementById('showJoke')
    let zarafat = '';
    const address = 'https://v2.jokeapi.dev/joke/Programming'
    // try{
    //     const response = await fetch(address)
    //     const data = await response.json()
    //     if(data.setup) {
    //         zarafat = `${data.setup}... ${data.delivery}`
    //     }
    //     else {
    //         zarafat = data.joke
    //     }
    //     meniGuldur(zarafat)
    //     showJoke.textContent = zarafat
    // }
    // catch(err) {
    // }
    
    /* Bu yuxaridaki funksiyani try/catch-siz de yazmaq olar.
       Cunki try/catch esasen custom errorlari yazan zaman istifade edilir.
       Asagida try/catch-siz yazmisam.
    */
...
function meniGuldur(joke) {
    const zarafatMetni = joke
    VoiceRSS.speech({
        key: '5d24449a50d14cf393d8108c149ed1b3',
        src: zarafatMetni,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
guldur.addEventListener('click',zarafatiYarat)
